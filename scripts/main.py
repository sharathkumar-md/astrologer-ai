...
class AstraBot:
    ...
    def get_location_data(self, location):
        ...
        if location_data:
            ...
            logger.info("  Coordinates: ({lat:.4f}, {lon:.4f})")
            logger.info("  Timezone: {tz_str}")
            return location_data
        
        logger.info("\n✗ Location '{location}' not found.")
        logger.info("Please try again with more details (e.g., 'City, State, Country')")
        return None
    ...
    def select_user(self):
        ...
        for user_id, name, birth_date in users:
            logger.info("{user_id}. {name} (Born: {birth_date})")
        
        logger.info("{len(users) + 1}. Create New User")
        ...
        if choice == len(users) + 1:
            ...
            if user_data:
                ...
                if self.conversation_history:
                    logger.info("Loaded {len(self.conversation_history)//2} previous conversations")
    ...
    def chat_loop(self):
        print("\n" + "-"*60)
        logger.info("You can now ask Astra anything about your life and emotions.")
